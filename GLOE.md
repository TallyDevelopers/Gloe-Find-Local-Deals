...
- **Outbound email is live via Resend** (`domain/email.ts` → `sendEmail()`, the single send door — no-ops if `RESEND_API_KEY` unset, never throws into the caller; React Email templates in `src/emails/`; recipient = user email, payer-email fallback for gift links). Sending domain `mail.gloe.app` (DKIM/SPF/DMARC verified); from `receipts@mail.gloe.app`, reply-to `support@gloe.app`. Resend stays in **testing mode** (verified-addresses-only) until toggled to production at launch. **Emails wired so far:**
  ...
  - **Welcome** (GLO-28) — `sendWelcomeEmail()` fires from the just-in-time user insert in `context/auth.ts` (first verified token → user row created → email), so it sends exactly once per signup, never on login; Resend idempotency key `welcome:<clerkUserId>` guards the double-insert race. **Deliberately a plain personal founder note** (from `Ryan at Gloē <ryan@mail.gloe.app>`, no branded shell/button/card) — the branded variant landed in Gmail's Promotions tab, and the promo signals ARE the layout, so don't "re-brand" it. Still fully static (no listings/prices), premium tone, "deals" absent; plain link → location-aware discover page (web root; swap to GLO-14 Universal Link when live). First name from Clerk, "Hi," fallback. Template `WelcomeEmail.tsx` (the only template NOT on `BaseLayout`, on purpose).
