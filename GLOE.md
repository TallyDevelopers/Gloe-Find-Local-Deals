...
- **Static top chrome** (3 rows): location bar · **category tabs** (`CategoryTabs` — text + underline, scrollable, the "what am I browsing" switch; same DB taxonomy via `useCategoryOptions`) · **filter pill row** `MapFilterChips` (Filter · Vibe · Price · Rating · Sort — "refine it"). The tab-vs-pill split (ResortPass pattern) is shared with the Discover screen for one coherent language. Every chip opens `MapFilterSheet` focused on its section; all map 1:1 onto `deals.list` inputs (`mapFilters.ts`), so applying is a spread. Vibe/rating/sort/price are **fully wired**, not stubs.
