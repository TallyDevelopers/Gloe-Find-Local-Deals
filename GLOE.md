...
- **Outbound email is live via Resend** (`domain/email.ts` → `sendEmail()`, the single send door; React Email templates in `src/emails/`). **Purchase receipts fire on fulfillment** (`fulfillPurchase`, fire-and-forget post-commit, idempotent), sent to the user (or payer for gift links), with the deal hero banner + voucher code(s) + "how to pull up your QR" (Wallet tab on mobile, `gloe.app/wallet` on web). Sending domain `mail.gloe.app` (DKIM/SPF/DMARC verified); from `receipts@mail.gloe.app`, reply-to `support@gloe.app`. Resend stays in testing mode (verified-addresses-only) until toggled to production at launch. Other transactional emails (refund/payout/gift/vendor-invite) + auth emails (Clerk) are still pending. No inbound parsing yet. Account deletion CASCADEs tickets+messages (PII).
