...
- **Outbound email is live via Resend** (`domain/email.ts` → `sendEmail()`, the single send door — no-ops if `RESEND_API_KEY` unset, never throws into the caller; React Email templates in `src/emails/`; recipient = user email, payer-email fallback for gift links). Sending domain `mail.gloe.app` (DKIM/SPF/DMARC verified); from `receipts@mail.gloe.app`, reply-to `support@gloe.app`. Resend stays in **testing mode** (verified-addresses-only) until toggled to production at launch. **Emails wired so far:**
  ...
  Still pending: payout-notification, gift-link confirmation, vendor-invite (GLO-17); auth emails are Clerk's (GLO-18). No inbound parsing yet. Account deletion CASCADEs tickets+messages (PII).
...
| **Claim flow (GLO-5)** | For admin-pre-created (unclaimed) vendors. `/vendor` silently runs `vendor.claimByEmail` before ever showing the signup form: if an unclaimed vendor's `email` matches one of the signed-in user's **verified** Clerk emails, `owner_user_id` links atomically and they land in their own dashboard (no duplicate vendor). Signup screen also carries a manual "claim your business" fallback. Audited (`vendor.claimed`). Code: `domain/vendorClaim.ts`. |
