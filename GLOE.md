...
| 7 | **ToS + Privacy Policy (hosted)** | App Store **requires** a Privacy Policy URL; "marketplace facilitator, not provider" clauses are legal protection for an aesthetic-services marketplace. ✅ **Drafts hosted (2026-06-11):** full consumer ToS at `/legal/terms`, privacy at `/legal/privacy`, and the **Vendor Agreement** (GLO-35 — facilitator + chargeback-liability + fee/offset clauses) at `/legal/vendor-terms`, accepted via a required checkbox at vendor signup (stamped `terms_accepted_at`/`terms_version`). **Remaining:** counsel review (GLO-15). | counsel pass | 🟡 Drafts live, counsel pending |
