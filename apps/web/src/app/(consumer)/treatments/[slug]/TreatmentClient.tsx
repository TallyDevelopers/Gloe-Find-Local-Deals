...
/**
 * Interactive treatment (category) listing. Rendered inside the server
 * `page.tsx` (which owns SEO metadata + JSON-LD). Desktop: a sticky, DATA-DRIVEN
 * filter rail (options derived from real inventory) + sorted grid. Mobile: the
 * rail collapses into a slide-up drawer. `slug` + `displayName` come from the
 * server so the heading renders in SSR HTML even before the client query lands.
 */
export function TreatmentClient({ slug, displayName: initialName }: { slug: string; displayName: string }) {
  ...
  const displayName = category?.displayName ?? initialName;
