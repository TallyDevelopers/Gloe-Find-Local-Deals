...
import { DealGridSkeleton } from './Skeletons';
...
/**
 * The full search experience — debounced live results, suggestions, recent +
 * trending, zero-state. Rendered two ways: as the /search page (no onClose, no
 * syncUrl=false) and inside the in-place SearchOverlay (onClose set). Keeping it
 * one component means both surfaces behave identically.
 *
 * - `initialQuery` seeds the field (from ?q= on the page).
 * - `syncUrl` keeps ?q= in the address bar in sync (page only; the overlay
 *   leaves the underlying URL alone so dismiss returns you where you were).
 * - `onClose` renders a leading close button and is called on result tap.
 */
export function SearchPanel({
  ...
  // Keep ?q= in sync for shareable URLs — page only. The overlay passes
  // syncUrl=false so it never rewrites the page you'll return to.
  useEffect(() => {
    if (!syncUrl) return;
  ...
  const results = trpc.deals.search.useQuery({ q: debounced, ...locArgs, limit: 40 }, { enabled: active });
  const suggestions = trpc.deals.suggest.useQuery({ q: debounced, ...locArgs, limit: 8 }, { enabled: active });
  const trending = trpc.deals.trending.useQuery({ ...locArgs, limit: 10 }, { enabled: !active });
  // Real nearby inventory for the idle state — photo cards say "look how much
  // is near you" far louder than word-chips do (the trust signal).
  const popular = trpc.deals.list.useQuery({ ...locArgs, limit: 6 }, { enabled: !active });
  ...
  return (
    <div className="consumer-container" style={{ maxWidth: 1100 }}>
      {/* Search field. (In the overlay, the close affordance + wordmark live in
          the overlay header — this is just the field.) */}
      ...
      {active ? (
        <div style={{ marginTop: 24 }}>
          {results.isLoading ? (
            ...
            <>
              ...
              <div className="deal-grid" onClick={onClose}>
        ...
        <div style={{ marginTop: 28, display: 'flex', flexDirection: 'column', gap: 28 }}>
          ...
          {/* Real inventory — the screen earns trust the moment you open it. */}
          {popular.isLoading ? (
            <Section title="Popular near you"><DealGridSkeleton count={4} /></Section>
          ) : popular.data && popular.data.deals.length > 0 ? (
            <Section title="Popular near you">
              <div className="deal-grid" onClick={onClose}>
                {popular.data.deals.slice(0, 6).map((deal) => (
                  <DealCard key={deal.id} deal={deal} />
                ))}
              </div>
            </Section>
